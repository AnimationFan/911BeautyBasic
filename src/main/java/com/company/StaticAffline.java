...
public class StaticAffline extends Affline{


    public StaticAffline(int[][] p, int[][] q, int height, int width, ImageOperator operator){
        super(p,q,height,width,operator);
    }

    public static int[] afflinePoint(int[] v,float[][] p,float[][] q,int height, int width){
        int ctrl = p.length;
        //前处理，应当放置于矩阵之外
    ...
    public void changeImage() throws InterruptedException {
        for(int i=0; i<this.height;i++){
            for(int j=0; j<this.width;j++){
                int[] point = afflinePoint(new int[]{i,j},this.p, this.q,this.height,this.width);
                operator.setPixelColor(i,j,point[0],point[1]);
            }
        }
    }
